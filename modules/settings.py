...
class Settings:
    ...
    def save_system_constants(self):
        try:
            file_path = self._get_resource_path("system_constants.json")
            with open(file_path, 'w', encoding='utf-8') as f: json.dump(self._system_data, f, indent=2, ensure_ascii=False)
            return True
        except Exception as e:
            print(f"Failed to save system_constants.json: {e}")
            return False
...
# Function form for backward compatibility
_settings = Settings()
...
def save_settings(): return _settings.save_system_constants()
